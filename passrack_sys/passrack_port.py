...
"""    
    ...
    ██╔═══╝░██╔══██║░╚═══██╗░╚═══██╗"""+colors.RED+"""  ██╔══██╗██╔══██║██║░░██╗██╔═██╗░"""+colors.STOP+"""
    ██║░░░░░██║░░██║██████╔╝██████╔╝"""+colors.RED+"""  ██║░░██║██║░░██║╚█████╔╝██║░╚██╗"""+colors.STOP+"""
    ╚═╝░░░░░╚═╝░░╚═╝╚═════╝░╚═════╝░"""+colors.RED+"""  ╚═╝░░╚═╝╚═╝░░╚═╝░╚════╝░╚═╝░░╚═╝""" + colors.STOP+'\n' + '\n passrack.py v1.8'+ '\n Created by: kuldeepkrjha && passrack team \n' + colors.normal
#ascii art by: kuldeepkrjha
os.system('clear')

quiet_banner = colors.red + '~ passrack ~' + colors.normal
...
def brute(service,port,fname,output,auserlist,ausername,apasslist,apassword,acontinuous,ahosts,athreads,averbose,acombo,adebug):
    if auserlist is None and ausername is None and acombo is None:
        ...
        userlist = '/usr/share/passrack/wordlist/'+service+'/user'
    ...
    if apasslist is None and apassword is None and acombo is None:
        passlist = '/usr/share/passrack/wordlist/'+service+'/password'
...
def parse_args():
    ...
    "Usage: python passrack.py <OPTIONS> \n")
    ...
    menu_group.add_argument('-o', '--output', help="Directory containing successful attempts", default="passrack-output")
...
if __name__ == "__main__":
    ...
    if args.modules is True:
        ...
        print(colors.normal + "\n") 
    try:
        tmppath = tempfile.mkdtemp(prefix="passrack-tmp")
    except:
        sys.stderr.write("\nError while creating passrack temp directory.")
        exit(4)
    ...
    if os.system("command -v medusa > /dev/null") != 0:
        sys.stderr.write("Command medusa not found. Please install medusa before using passrack")
        exit(3)
